...
ai_ingestion_guidance:
  ...
  - "Record scope and legal limitations separately. An in-scope subject may also have a limitation affecting only part of its obligations or operating scope."
---
...
## Permitted scope outcomes

The gate must produce one of the following scope outcomes. Legal limitations are recorded separately and must not replace the underlying scope decision.
...
### Separate legal-limitation status

After recording the scope outcome, use one of these limitation statuses:

| Status | Meaning |
|---|---|
| `none identified` | No applicable exclusion, exemption, transition, fundamental-alteration decision, disproportionate-burden decision, or other legal limitation has been established. |
| `one or more limitations recorded` | A legally controlled limitation affects defined obligations, functions, territories, periods, or parts of the subject. The underlying scope outcome remains visible. |
| `limitation review required` | A possible limitation has been raised but its legal basis, evidence, authority, conditions, or affected scope has not been established. |

A decision can therefore be `in scope` with `one or more limitations recorded`. A limitation affecting one part of a service must not be generalised to unaffected functions, channels, territories, periods, or continuing duties.
...
| Related inclusive-outcome IDs | Relevant `IDSO` outcome identifiers from the canonical Layer 2 dataset; these support design and evidence planning and do not determine legal applicability. |
...
decision_id: "EAA-AP-"
version: ""
record_status: "draft | approved | superseded | withdrawn"
assessment_date: ""
product_or_service: ""
feature_or_channel: ""
...
included_scope: []
excluded_scope: []
scope_decision: "in scope | partly in scope | out of scope | legal review required"
legal_limitation_status: "none identified | one or more limitations recorded | limitation review required"
legal_limitations:
  - limitation_id: ""
    limitation_type: ""
    affected_scope: []
    ...
    evidence_references: []
    decision_authority: ""
    notification_or_publication_duties: []
    continuing_duties: []
    approval_date: ""
    review_date: ""
    expiry_date: ""
...
- [EAA Evidence and Conformity Record Model](EAA_Evidence_and_Conformity_Record_Model.md)
